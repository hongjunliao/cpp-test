...
{
	if(objsize <= 0) return (struct mmpool *)0;
	if(!(factor >=1 && factor <= 10)) factor = 5;

	page_size = sysconf(_SC_PAGESIZE);

	struct mmpool * mp = (struct mmpool* )calloc(1, sizeof(struct mmpool));
	if(!mp) return 0;
	...
	mp->stat_memused = sizeof(struct mmpool) + mp->X * sizeof(void *);
	if(verbose > 0)
...
int mmp_test_main(int argc, char ** argv)
{
	...
	mmp_set_loglevel(1);
	struct mmpool * mp = mmp_create(sizeof(struct list_node), 10);
	if(!mp) return -1;
	...
	for(; i < n; ++i){
		...
		if(method == 0)
			node = (struct list_node *)mmp_alloc(mp);
		else
